...
class LearningRateMonitor(Callback):
  ...
  def __init__(
    ...
    super().__init__(callback_config, trainer)
...
class GradientSpikeMonitor(Callback):
  ...
  def __init__(
    self, config: CallbackConfig, gs_config: GradientSpikeConfig, trainer: TrainerModules
  ):
    super().__init__(config, trainer)
    assert self.trainer.trainer_config.log_grad_norm, (
  ...
  def on_training_step(
    self, trainer_state, step_metrics, epoch_idx: int, step_idx: int
  ) -> None:
  ...
  def on_filtered_training_epoch_end(self, train_metrics, epoch_idx) -> None:
    del train_metrics
  ...
  def log_gradients_spikes(self, num_elements: int, epoch_idx: int) -> None:
    ...
    losses_spike = losses_spike & np.logical_not(
      np.concatenate([[False], losses_spike[:-1]])
    )
    # Log the spikes.
    ...
    self.trainer.logger.log_scalar(
      "optimizer/spikes_losses", losses_num_spikes, epoch_idx
    )
